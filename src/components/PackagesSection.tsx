...
const PackagesSection: React.FC = () => {
  ...
  const loadCapacityInfo = async () => {
    try {
      setIsLoading(true);
      const info = await supabaseService.getPackageStatistics();
...
const PackageCard: React.FC<PackageCardProps> = ({ package: pkg, index, capacityInfo, isLoading }) => {
  ...
  return (
    ...
    >
      ...
      {!isLoading && capacityInfo && (
        <div className={`absolute top-16 left-4 text-xs font-bold px-2 py-1 rounded-full z-10 ${
          capacityInfo.isFull 
